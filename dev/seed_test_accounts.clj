(ns dev.seed-test-accounts
  "CLI helper to seed or check test accounts in Datomic.

  Usage:
    lein run -m dev.seed-test-accounts            ; uses dev/test-accounts.edn if present
    lein run -m dev.seed-test-accounts path.edn   ; custom path

...
- **Export runs the same checks as import** — duplicates/cleanups caught on the way out, on every export: one source or the whole library, plain or pretty-printed. Emergency raw export is the one unchecked escape hatch, offered only when validation hard-fails (`9df1b4ae`, `c037de78`).

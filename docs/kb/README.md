...
| [dropdown-value-coercion.md](dropdown-value-coercion.md) | The `<select>`-always-yields-a-string footgun (~70 call sites coerce by hand; one forgot → silent corruption) and the `:typed?` template that round-trips the value's type. Decision **D32**. | High — repo-wide census + E2E-backed |

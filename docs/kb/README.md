...
| [source-tagged-keys.md](source-tagged-keys.md) | **Decided, not built:** every minted key carries its source's abbreviation (`:stone-elf-tc`, `dflt` for the default source), so duplicates mostly stop happening and overriding an SRD item becomes the explicit act of deleting the tag. The mechanism (`common/disambiguated`) ships already and runs only on collisions. | Decided 2026-09-13 |
...
| [already-held-grants.md](already-held-grants.md) | **Read before changing how a grant lands.** What happens when a grant duplicates something the character already has: four paths, three behaviours, only `background-skills-cfg` correct. Measured per pool. Also records that a race's `:props` are written by six builder widgets and read by nothing. | High — built characters |

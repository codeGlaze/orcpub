...
| [source-tagged-keys.md](source-tagged-keys.md) | **OPEN question, nothing built:** should every minted key carry its source's abbreviation (`:stone-elf-tc`), so duplicates mostly stop happening and overriding an SRD item becomes an explicit act of deleting the tag? The mechanism (`common/disambiguated`) ships already and runs only on collisions. | Decision pending — blocked on the default source |

...
| [homebrew-reference-web.md](homebrew-reference-web.md) | **Read before rename, move, delete, disable, import conflict resolution or share bundling.** Every link between homebrew items (thirteen kinds, by key or by name; every one probed under a rename by `reference_web_test`), the links into homebrew from characters, custom items and encounters, the full spell-list model (three sources, the subclass correction, the spell/class cycle), and which consumer knows which links. Delete checks none; rename knows two, and `reference_web_test` pins the other eleven. | High — traced, every row re-read in code |
...
| [garden-inline-styles-harvest.md](garden-inline-styles-harvest.md) | Why `refactor/garden-inline-styles` is harvested rather than merged into integration: the measured conflicts (648 commits of drift, a 256-line and a 72-line hunk), the 72% of the conversion judgement that still transfers, the one header fix integration still lacks, and the two fixes deliberately left because integration already solves them better |

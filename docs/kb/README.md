...
| [requirements-registry.md](requirements-registry.md) | **DESIGN — read before adding a conditional effect or a "while X" anything.** One registry of facts content asks about (`:dual-wielding?`, `:unarmored?`), with three gates — `:build` computes, `:toggle` is player-asserted, `:text` is a trigger that prints and never computes. The dual-wielding evidence (one fact, three hand-written sites, two effect channels), the naming and what lost, the prereq kinship, and the contract change that blocks it. | DESIGN — evidence verified |

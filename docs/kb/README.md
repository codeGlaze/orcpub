...
| [homebrew-reference-web.md](homebrew-reference-web.md) | **Read before rename, move, delete, disable, import conflict resolution or share bundling.** Every link between homebrew items (twelve kinds, by key or by name), the links into homebrew from characters, custom items and encounters, the full spell-list model (three sources, the subclass correction, the spell/class cycle), and which consumer knows which links. Delete checks none; rename knows two. | High — traced, every row re-read in code |

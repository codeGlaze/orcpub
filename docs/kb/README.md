...
| [class-feature-catalogue.md](class-feature-catalogue.md) | Per-class inventory of all 10 base classes (roadmap C1): distinct auto-features, sizing (~3–6 each; monk/paladin outliers, sorcerer/wizard lean), and the cross-cutting odd cases the registry must handle — multi-source use-counts, class-wide resource pools (ki/sorcery/Lay-on-Hands), build-context summary interpolation, multi-part features, attribute interdependence. | High — all 10 class option fns read, file:line cited |

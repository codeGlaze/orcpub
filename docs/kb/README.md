...
| [declarative-grant-vocabulary.md](declarative-grant-vocabulary.md) | DESIGN: a builder-UI vocabulary (`<grant spell>` creator-fixed / `<select spell>` user-choice + filters; `<grant X when [conditions]>`) compiling to the existing primitives. Analysis: it works for spell patterns and subsumes the ~18 hardcoded feat templates; special cases (spellcasting progression, usage qualifiers) flagged OPEN. | Mixed — verified findings + flagged DESIGN/OPEN |

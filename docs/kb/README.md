...
| [ability-increase-spreads.md](ability-increase-spreads.md) | `:ability-increases` as terse `[amount pool]` pairs — fixed/floating/Tasha's "+2/+1"/arbitrary custom spreads, with the "different abilities" rule. Compile + assign-from-bag widget + authoring. Decisions **D33** (terse export data). | High — JVM + cljs + E2E-backed |

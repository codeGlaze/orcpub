...
| [icon-font-failure.md](icon-font-failure.md) | When an icon font fails the control does not degrade, it disappears: zero-width, unclickable, absent from the accessibility tree. Which detection methods actually work (`fonts.check()` and the glyph-width trick do not), why a computed colour absent from source means a browser extension is rewriting CSS, and why interactive icons should not depend on a webfont |

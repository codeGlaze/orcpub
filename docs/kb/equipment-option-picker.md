...
*Analysed 2026-09-06 on `feat/option-picker`. Sources: the browser probes named at the
bottom, run against `lein e2e-server` with the WotC megapack imported; DOM counts and
longest-task figures are from those runs, not estimates.*
...
User-Agent based, so a narrow desktop window renders the DESKTOP layout squeezed, not the
mobile one; only a mobile UA gets the mobile builder).
...
else is under ~40 options, where a native `<select>` is the right control. Recorded as a
follow-up in `docs/TODO.md` on `feat/option-picker`; that file on this branch predates it.
...
`option_menu_views` came from `redesign/growable-option-menus` -- see
[growable-option-menus.md](growable-option-menus.md) for that effort's continuity record.

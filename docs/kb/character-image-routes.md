...
**Analysis date:** 2026-09-06. **Based on:** live requests to the hosts named below
(curl and a Playwright-driven Chromium), the running app on `lein e2e-server`, and
`test/browser/character_image_capture_e2e.js`. Every number here came from a
request that was actually made; nothing is inferred from documentation.

The runbook for operating the feature is `docs/CHARACTER-IMAGE-FETCH.md` on
`integration`; this file is what was learned getting there, including the wrong
turns, because two of them cost a day and both are easy to repeat.
...
⚠️ UNVALIDATED SPECULATION — the five hosts marked "untested" for the server column
were never fetched with a real URL. They are listed only because they send no ACAO,
which says nothing about whether they would serve this server. Do not repeat the
mistake below by assuming.

# Branch context history — the previous BRANCH.md (2026-06-14 → 2026-06-18)

**History, preserved verbatim.** This was this branch's `BRANCH.md` from `5a33faa9` to `55e796e8`,
written while the work ran under the harness branch `claude/zen-wright-04xhdz`. It was replaced on
2026-09-25 when the agent docs moved to `agents/develop`'s three tiers (see `BRANCH.md`).

The items in it that are still live were lifted into `BRANCH.md`: the deferred follow-ups to
surface at branch close, the working agreements, and the two standing rules for identity and
catalogs. Everything else below is history — its "NEXT" lines and phase checklist are superseded
by `roadmap.md` and `plan-next.md`, and it says so itself.

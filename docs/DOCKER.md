...
The setup and management scripts (`docker-setup.sh`, `docker-user.sh`,
...
```sh
./docker-setup.sh          # prompts for each setting
```
...
```sh
./docker-setup.sh --auto    # creates .env with generated passwords
```
...
| `docker-setup.sh` | Interactive setup: generates `.env`, dirs, SSL certs, secrets |
...
- Missing `SIGNATURE` (set it in `.env` or `docker-setup.sh` generates one)

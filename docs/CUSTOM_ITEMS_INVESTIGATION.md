...
Originally written on `claude/fix-custom-items-disappearing-DW8rb`, which was
re-homed as `fix/filtered-list-staleness` to be mergeable.

## STATUS: merged into integration

These are the working notes as they stood during the investigation, kept for the
reasoning behind P1-P5. They are a historical record, not a plan: where they
describe work as pending, or route the Playwright spec to `testing/develop`, that
is what was expected at the time and not what happened. The spec never became
runnable and did not land.

What actually verified the fix is on `agents/develop` under `docs/kb/`:
`filtered-list-staleness.md` (the defect), `plan-669-merge-verification.md` (the
plan and its results), and `handoff-669-final-pass.md`. The regression net that
did land is `test/cljs/orcpub/dnd/e5/filtered_list_reactivity_test.cljs`.

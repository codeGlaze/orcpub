...
| | | status |
|---|---|---|
| 1 | **CR as one number** — one `double`, one formatter, one parser. Shape: [decision-cr-representation.md](kb/decision-cr-representation.md). Execution: [plan-cr-normalization.md](kb/plan-cr-normalization.md), six steps. **Step 0 done** — `cr_characterization_test.clj`, proven able to fail against the real data. Blast radius checked — encounters reference creatures by keyword, the monster builder already writes doubles, real `.orcbrew` content is already decimal | ☐ |

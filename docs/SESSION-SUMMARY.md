...
## Lint Cleanup & Entity/Build Investigation (February 2026 - 10th Session)

Fixed lint warnings across the codebase, resolved clojure-lsp false positives from compiled CLJS output, and investigated the zero-coverage `entity/build` pipeline.

### clojure-lsp False Positives Fix

**Problem**: clojure-lsp scanned `resources/public/js/compiled/out/` (compiled CLJS output), producing hundreds of `unused-public-var` warnings from third-party code like `cljs_time/core.cljs`.

**Root cause**: `resources` is on the classpath via `:resource-paths` in `project.clj`. `source-paths-ignore-regex` filters directory-level source paths, not files within directories — the discovered path is `resources`, which doesn't match `"resources/public/js/compiled"`.

**Fix**: Explicit `:source-paths` in `.lsp/config.edn` overrides all classpath discovery:
```edn
:source-paths #{"src/clj" "src/cljc" "src/cljs" "web/cljs"
                "test/clj" "test/cljc" "test/cljs" "dev"}
```

### clj-kondo Config Fixes

| Issue | Fix |
|-------|-----|
| `exclude-when-defined-by [user]` (wrong semantics) | Changed to `{:exclude [user]}` — excludes all vars in namespace |
| `orcpub.styles.core/app` flagged unused | Added to `:exclude` — it's the garden entry point from `project.clj` |

### Lint Fixes Applied

| File | Issue | Fix |
|------|-------|-----|
| `styles/core.clj` | `sm-or-md-query` unused private var | `#_` commented (only used in commented block) |
| `styles/core.clj` | `xs-min` unused var | `#_` commented (defined but never referenced) |
| `common.cljc:66` | Redundant nested `(str ...)` | Removed inner str, merged args into outer str |
| `magic_items.cljc:1445` | `(str "literal")` wrapping string | Removed str wrapper |
| `template_base.cljc:72` | Nested `(+ x (+ a b c) y)` | Flattened to `(+ x a b c y)` |
| `warlock_test.clj` | Unused requires (deftest, is, union) | `#_` commented entire `:require` block |

### Entity/Build Coverage Gap Discovery

**Finding**: `entity/build` — the core character-building function — has **zero test coverage**. Every character sheet goes through it, but no test exercises it.

The disabled `warlock_test.clj` was the only test that called `entity/build`. It fails because all 12 `template-selections` params are `nil`, producing an empty template with no modifiers.

### Knowledge Base Files Created

| File | Content |
|------|---------|
| `docs/TESTING.md` | Test suite inventory, coverage gaps, disabled tests, patterns, linter config, gotchas |
| `docs/ENTITY-BUILD.md` | Build pipeline architecture, template-selections 12 params, data sources (cljc vs cljs-only), option builder signatures, modifier gotchas |

### Files Modified

| File | Change |
|------|--------|
| `.lsp/config.edn` | Explicit source-paths to stop scanning compiled CLJS |
| `.clj-kondo/config.edn` | Fixed unused-public-var exclude, added `app` exclude |
| `src/clj/orcpub/styles/core.clj` | `#_` commented `sm-or-md-query` and `xs-min` |
| `src/cljc/orcpub/common.cljc` | Removed redundant nested str |
| `src/cljc/orcpub/dnd/e5/magic_items.cljc` | Removed redundant str wrapper |
| `src/cljc/orcpub/dnd/e5/template_base.cljc` | Flattened nested + |
| `test/cljc/orcpub/dnd/e5/warlock_test.clj` | `#_` commented all forms (pending rewrite) |

### Warlock Test Rewrite (completed)

Rewrote `warlock_test.clj` as a working `entity/build` integration test:
- 7 tests, 20 assertions — all passing
- Uses real cljc data modules (`spell-lists`, `spell-map`, `weapons-map`, `weapons`)
- Inline configs for cljs-only data (elf/drow race, spy background, keen mind feat, language map)
- Calls `classes5e/warlock-option` directly — same code path as production
- Fixed `::char5e/str` shadowing bug by using explicit key access
- Removed `great-weapon-master` feat (no config available, would be silently skipped)

Key findings during implementation:
- `feat-option-from-cfg` uses `:ability-increases` (a SET), not `:modifiers` — different format from `feat-option`
- Warlock spells use schedule mode — regular cantrips/spells don't appear in `spells-known`
- `entity.cljc` auto-flattens nested modifier vectors at lines 553/589
- Unmatched entity options are silently skipped (no error)
- All built-in `feat-options` return `[]` (every feat is `#_` commented)

Full suite: 74 tests, 237 assertions, 0 errors. The 1 failure (`strict-round-trip-2`) is pre-existing.

---

## strict-round-trip-2 Fix & cljs Data Sharing Research (February 2026 - 12th Session)

Fixed the `strict-round-trip-2` test failure and researched whether .clj tests can access data from .cljs files.

### strict-round-trip-2 Fix

**Location**: `character_test.clj:105-108`

**What it tests**: Round-trip conversion of a character entity through strict format:
`strict → char/from-strict → char/to-strict → should equal original`

**Two bugs found and fixed**:

1. **Selection ordering lost** (`entity.cljc:167`): `from-strict-selections` used `reduce`/`assoc`
   starting from `{}`. Beyond 8 keys, Clojure promotes `PersistentArrayMap` to `PersistentHashMap`,
   losing insertion order. The test data has 16 selections.
   - **Fix**: Changed to `apply array-map` + `mapcat`, which creates a `PersistentArrayMap`
     of any size preserving insertion order. Downstream `update-in` calls (e.g. `vectorize-equipment`)
     only update existing keys, so no promotion occurs.

2. **Owner field dropped** (`entity.cljc:138`): `to-strict` destructured `[:db/id ::options ::values
   ::homebrew-paths]` but NOT `::owner`. The `from-strict` function saves `::strict/owner` →
   `::owner`, but `to-strict` never converted it back.
   - **Fix**: Added `::owner` to destructuring and `owner (assoc ::strict/owner owner)` to `cond->`.

**Result**: 74 tests, 237 assertions, **0 failures**, 0 errors. Previously 1 failure.

### cljs Data Sharing: Can .clj Tests Access .cljs Data?

**Answer: NO currently, but the data doesn't need to be in .cljs at all.**

**Why .clj can't require .cljs**: Different compilation targets. `lein test` runs the Clojure compiler (JVM), which cannot load `.cljs` files. The `.cljs` files are only loaded by the ClojureScript compiler.

**Key finding**: The data "stuck in cljs" (`spell_subs.cljs`) has NO cljs-specific dependencies. Every dependency is already in `.cljc`:
- `languages` (line 476) — pure data, zero dependencies
- `acolyte-bg` (line 444) — uses `opt5e/*` and `equipment5e/*`, all in `.cljc`
- `elf-option-cfg` (line 555) — uses `opt5e/*`, `mod5e/*`, all in `.cljc`
- Helper functions (`high-elf-cantrip-selection`, `elf-weapon-training-mods`) — only use `.cljc` modules

The ONLY cljs-specific import is `re-frame.core/reg-sub`, used for UI subscriptions — not for data definitions.

**Recommended refactoring**: Extract pure data/functions from `spell_subs.cljs` into `.cljc` modules:
- `languages.cljc` — language data (file exists, currently empty/specs only)
- `backgrounds.cljc` — background configs (file exists, currently empty/specs only)
- `races.cljc` or new `race_configs.cljc` — race option configs
- Keep only `(reg-sub ...)` calls in `spell_subs.cljs`, requiring the new `.cljc` modules

**Benefits**: Single source of truth, zero test data duplication, `.clj` tests require from `.cljc` directly.

### Files Modified

| File | Change |
|------|--------|
| `src/cljc/orcpub/entity.cljc:167` | `from-strict-selections`: `reduce`/`{}` → `apply array-map`/`mapcat` (with comment) |
| `src/cljc/orcpub/entity.cljc:138` | `to-strict`: added `::owner` → `::strict/owner` round-trip (with comment) |

### Worktree Created

| Worktree | Branch | Base | Purpose |
|----------|--------|------|---------|
| `/workspaces/orcpub` | `upgrade/datomic-pro` | — | Main work: Datomic upgrade, tests, bug fixes |
| `/workspaces/orcpub-debug` | `debug/develop-01` | — | Debug/investigation |
| `/workspaces/orcpub-refactor` | `refactor/cljs-data-to-cljc` | `origin/develop` | Extract pure data from .cljs → .cljc |

### Key Insight: Strict Format Serialization Chain

`from-strict` / `to-strict` is the **Datomic persistence layer**, not UI-related:
- `strict format` (vectors, ordered, with `:db/id`) ↔ `entity format` (maps, random access)
- Characters are stored in Datomic in strict format, converted to entity format for in-memory use
...
### Key Insight: PersistentArrayMap Threshold

Clojure's map implementation switches at 8 keys:
- ≤8 keys: `PersistentArrayMap` — preserves insertion order, O(n) lookup
- \>8 keys: `PersistentHashMap` — arbitrary iteration order, O(log32 n) lookup
- `(array-map ...)` and `(apply array-map ...)` force `PersistentArrayMap` at any size
- `assoc` on `PersistentArrayMap` with >8 keys promotes to `PersistentHashMap` (new keys only; updating existing keys stays as `PersistentArrayMap`)

### Handoff: refactor/cljs-data-to-cljc Branch

**Goal**: Extract pure data definitions from `spell_subs.cljs` into `.cljc` modules for single-source-of-truth.

**Scope** (all in `/workspaces/orcpub-refactor`):

1. **Languages** → `src/cljc/orcpub/dnd/e5/languages.cljc` (file exists, currently specs only)
   - Source: `spell_subs.cljs:476-508` — `(def languages [...])`
   - Pure data, zero dependencies

2. **Background configs** → `src/cljc/orcpub/dnd/e5/backgrounds.cljc` (file exists, currently specs only)
   - Source: `spell_subs.cljs:444-466` — `(def acolyte-bg {...})`
   - Dependencies: `opt5e/*`, `equipment5e/*` — all already `.cljc`

3. **Race configs** → new file or extend `src/cljc/orcpub/dnd/e5/races.cljc`
   - Source: `spell_subs.cljs:555-591` — `(defn elf-option-cfg [spell-lists spells-map language-map] ...)`
   - Also move helpers: `high-elf-cantrip-selection`, `elf-weapon-training-mods`
   - Dependencies: `opt5e/*`, `mod5e/*` — all already `.cljc`

4. **Update spell_subs.cljs** — keep only `(reg-sub ...)` calls, require new `.cljc` modules

5. **Update warlock_test.clj** (on `upgrade/datomic-pro` after merge) — replace inline configs with requires from new `.cljc` modules

**Verification**:
- `lein test` (JVM tests pass with new `.cljc` requires)
- `lein cljsbuild once dev` (ClojureScript build still works with `spell_subs.cljs` requiring from `.cljc`)
- No functional changes — pure data reorganization

**Risks**:
- `spell_subs.cljs` is large (~900 lines). Need to identify ALL data/helper functions vs subscription code
- Some helper functions may have subtle dependencies not caught in the initial analysis
- ClojureScript build may have different classpath resolution — verify with `cljsbuild`

### Session Compaction
- **February 2026 (10th)**: Lint cleanup, LSP config fix, entity/build investigation, KB file creation
- **February 2026 (11th)**: Warlock test rewrite, KB updates with implementation insights
- **February 2026 (12th)**: strict-round-trip-2 fix (two bugs), cljs data sharing research, refactor branch + worktree created

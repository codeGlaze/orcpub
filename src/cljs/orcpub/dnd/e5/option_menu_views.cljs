(ns orcpub.dnd.e5.option-menu-views
  "Shared view layer for the growable multi-select menus.
   ...
   DIVERGES from port/redesign-on-refactor: adds `:max-rendered`. Without it this renders
   every option as live DOM, which is fine for the builder menus it was written for (tens)
   and slower than a native <select> for inventory lists (hundreds)."
...
(defn option-menu
  ...
  [{:keys [menu-id title top-level? wildcards slot-label options header-extra
           multiselect? collapsible? on-clear cell-fn chip-fn trailer max-rendered]
    :or   {multiselect? true}}]
  (let [collapsible? (boolean (and title collapsible?))
        ...
        filtered  (if (str/blank? q)
                    ...
                    (filterv (fn [{:keys [display label]}]
                             ...
                             annotated))
        ;; :max-rendered caps how many options reach the DOM. A native <select> never
      ...
      [:div.opt-section-body-inner
       ...
       (when has-opts?
         (cond
           ...
           (= layout :pills) [pills-body shown cell-fn]
           (= layout :az)    [az-body menu-id annotated shown cell-fn]

...
(defn save-collision
  ...
  (let [occupant (get-in plugins [option-pack plugin-key key])
    ...
    ;; Refusing it fixed nothing and trapped the item: since the key is minted once, the author
    ;; cannot rename their way out either, because renaming no longer moves the key. The library
    ;; health card is where a standing duplicate is reported; the save is not.
    (when-not self?
      (if occupant

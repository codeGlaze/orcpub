...
(ns orcpub.routes.share
  "The browser compresses a character's homebrew bundle, encrypts it with a key derived from the
   ...
   max-blob-chars a snapshot, the newest max-shares-per-character kept for each character, and
...
;; Measured 2026-09-13 against a 12-source content pack: a packed level 20 character's snapshot
;; stores 21 KB (artificer) to 32 KB (wizard with 40 homebrew spells), and a snapshot of the whole
;; pack would store about 560 KB. The browser refuses to build one over max-blob-chars.
(def max-blob-chars (* 256 1024))
(def max-shares-per-character 10)
(def max-bytes-per-owner (* 5 1024 1024))

;; Version 2 is the encrypted snapshot; version 1 is the payload embedded in a link.
(def ^:private blob-shape #"2[A-Za-z0-9_-]+")

(defn share-id
  "The id a blob is stored under: the first 22 characters of its SHA-256 in base64url. The browser
   computes the same, so the id in a link names exactly one blob."
  [^String blob]
  (let [digest (.digest (MessageDigest/getInstance "SHA-256") (.getBytes blob "UTF-8"))]
...
(defn- read-blob
  "The request body as text, or ::too-large once it passes max-blob-chars. Reads at most one
   character past the limit, so an oversized body is not buffered whole."
  [body]
  (when body
    (with-open [r (io/reader body :encoding "UTF-8")]
      (let [buf (char-array (inc max-blob-chars))
            n   (loop [off 0]
                  (if (= off (alength buf))
                    off
                    (let [got (.read r buf off (- (alength buf) off))]
                      (if (neg? got) off (recur (+ off got))))))]
        (if (> n max-blob-chars) ::too-large (String. buf 0 (int n)))))))
...
(defn get-share
  "A snapshot's ciphertext as text, to anyone who asks."
  [{:keys [db] {:keys [id share]} :path-params}]
  (if-let [blob (d/q '[:find ?blob . :in $ ?rid
                       :where [?e :orcpub.share/id ?rid] [?e :orcpub.share/blob ?blob]]
                     db (record-id id share))]
    {:status 200 :headers {"Content-Type" "text/plain; charset=utf-8"} :body blob}
    {:status 404}))

(defn put-share
  "Stores a snapshot for a character the caller owns. The body is the blob as text, and the path names
   the id it must hash to. Stores nothing when that snapshot is already there. Anyone else gets the 404
    ...
    (if-not (owns-character? db username id)
      ...
      (let [blob (read-blob body)]
        (cond
          ...
          (not (and (string? blob) (re-matches blob-shape blob)))
          ...
          (let [existing (sort (d/q '[:find [?e ...] :in $ ?c :where [?e :orcpub.share/character ?c]] db id))
                ...
                used     (or (ffirst (d/q '[:find (sum ?size) :with ?e :in $ ?owner
                                          ...
                                          db username))
                             0)]
            (if (> (+ (- used (reduce + (map size-of evicted))) (count blob)) max-bytes-per-owner)
              ...
              (do @(d/transact conn (concat (map (fn [e] [:db/retractEntity e]) evicted)
                                            [{:orcpub.share/id        (record-id id share)
                                              ...
                                              :orcpub.share/blob      blob
                                              :orcpub.share/size      (count blob)}]))
                  {:status 200 :body {:share share}}))))))))

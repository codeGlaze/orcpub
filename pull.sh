...
load_state() {
  if [[ -f "$STATE_FILE" ]]; then
    while IFS='=' read -r key val; do
      ...
      val="${val%\"}"
      case "$key" in
        LAST_INTEGRATION_BRANCH) LAST_INTEGRATION_BRANCH="$val" ;;
        LAST_TESTING_BRANCH) LAST_TESTING_BRANCH="$val" ;;
        LAST_WORKING_BRANCH) LAST_WORKING_BRANCH="$val" ;;
      esac
    done < "$STATE_FILE"
  fi
}
...
# Ensure state is saved on any exit (success, error, or interrupt)
trap save_state EXIT

ensure_clean_worktree() {
  if [[ -n $(git status --porcelain) ]]; then
    echo -e "${YELLOW}Working tree is not clean. Please commit or stash changes first.${RESET}"
    exit 1
  fi
}
...
echo -e "${SEPARATOR}"
if [[ "$CURRENT_BRANCH" =~ ^integrat(e|ion)/ || "$CURRENT_BRANCH" =~ ^testing/ ]]; then
...
if [[ -z "$WORKING_BRANCH" ]]; then
  ...
  if [[ "$WORKING_BRANCH" =~ ^remotes/origin/ ]]; then
    ...
    if git show-ref --verify --quiet "refs/heads/$LOCAL_BRANCH_NAME"; then
      echo "Local branch $LOCAL_BRANCH_NAME already exists; switching to it."
      git checkout "$LOCAL_BRANCH_NAME"
    else
      git checkout --track -b "$LOCAL_BRANCH_NAME" "origin/$LOCAL_BRANCH_NAME"
    fi
...
if ! git merge origin/"$TESTING_BRANCH"; then
  echo -e "${YELLOW}Merge conflicts detected. Please resolve them and re-run the script.${RESET}"
fi
...
if git show-ref --verify --quiet refs/remotes/origin/agents/develop; then
  ...
  if ! git merge origin/agents/develop; then
    echo -e "${YELLOW}Merge conflicts detected. Please resolve them and re-run the script.${RESET}"
  fi
else
...
# Merge working branch: prefer local branch if present
echo -e "${SEPARATOR}"
echo -e "${BOLD}Merging from:${RESET} ${GREEN}$WORKING_BRANCH${RESET}"
if git show-ref --verify --quiet "refs/heads/$WORKING_BRANCH"; then
  # Local branch exists - merge it directly (may have unpushed commits)
  if ! git merge "$WORKING_BRANCH"; then
    echo -e "${YELLOW}Merge conflicts detected. Please resolve them and re-run the script.${RESET}"
  fi
else
  # No local branch - fetch and merge from remote
  git fetch origin "$WORKING_BRANCH"
  if ! git merge origin/"$WORKING_BRANCH"; then
    echo -e "${YELLOW}Merge conflicts detected. Please resolve them and re-run the script.${RESET}"
  fi
fi
